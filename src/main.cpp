...
int solve_file(std::string inputfile, std::string outputfile)
{
    ...
    if (input.is_open() and output.is_open())
    {
        ...
        std::cout << std::endl << "Solved " << cases << " puzzles in "<< elapsed.count() * 1e-9 << " seconds." << std::endl;
    }
    else
    {
        return -1;
...
void helptext(char **argv){
    ...
    std::cout << "\t" << argv[1] << " [options] inputfile [outputfile]" << std::endl;
    std::cout << "\t" << argv[1] << " [options] puzzle" << std::endl;
